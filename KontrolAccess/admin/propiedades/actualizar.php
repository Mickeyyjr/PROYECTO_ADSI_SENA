...
<?php 
    ...
    //Leer los datos del usuario a actualizar a traves de la url

    $id = (int)$_GET['id']; //convertir variable a un entero (integer)
    ...
    $consulta4 = "SELECT * FROM usuario WHERE id_usuario = $id";
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        //Validador de campos, si estan vacios no se ejecuta la insercion de datos en la db
            ...
            if(empty($errores)){
                ...
                $query = "UPDATE usuario  SET tipo_doc = '$tipo_doc', documento = '$documento', nombres = '$nombres', apellidos = '$apellidos', email = '$email', id_rol = '$rol', id_jornada = '$jornada' WHERE id_usuario = $id;";
                $result = mysqli_query($db, $query);

                 //Redireccionar cuando se haya registrado correctamente un usuario, mandando una respuesta a traves de la url
                if($result){
                header('Location: ../index.php?request=2'); //se manda una respuesta a traves de la url donde se va a leer en el index.php
                 }
            }
    }
...
<a href="../propiedades/editar.php" class="btn">Volver</a>
    ...
    <div class="formulario contenedor-form">
        <form  class="formulario-registro" method="POST" >
            ...
            <div class="inicia-sesion">
                <input type="submit" name="enviar" value="Actualizar usuario" class="btn ">
            </div>
